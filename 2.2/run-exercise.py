#!/usr/bin/env python3
"""
Exercise-2.2 — TCP Session Filtering (sin ONOS)

Sin ONOS, las sesiones TCP se registran manualmente via CLI (mismo P4,
pero sin la app Java TCPSessionManager que detectaría SYN automáticamente).
El data plane sigue generando RST para paquetes TCP fuera de sesión.
...
Uso:
  ...
  4. En la CLI de Mininet:
       mininet> pingall                         (ICMP no es TCP, debería funcionar)
       mininet> h2 /usr/sbin/sshd &             (levantar SSH en h2)
       # Antes de conectar SSH, registrar la sesión TCP en s1-commands.txt
       # o usar add_tcp_session.sh (ver documentación)
"""
...
def main():
    ...
    print("  1. Registrar sesión SSH (ver add_tcp_session.sh)")
